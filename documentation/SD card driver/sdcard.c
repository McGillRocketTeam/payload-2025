...
FIL fil;
UINT line_written;
const UINT SD_FLUSH_LINE_SIZE = 4;
const UINT SD_FLUSH_TIME_FREQ_SEC = 3;

/* Private variables */
uint8_t flush_timer = 0;
	...
	if (htim == &htim14)
	{
		if (flush_timer == 3)
		{
			if (line_written > 0)
			{
				...
				if (fres == FR_OK)
				{
					myprintf("Flushed after %i seconds of inactivity\r\n", SD_FLUSH_TIME_FREQ_SEC);
					line_written = 0;
				}
				else
				{
