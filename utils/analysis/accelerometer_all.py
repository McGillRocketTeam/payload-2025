...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("AccelerometerAllPlotter")
    parser.add_argument("dir", help="Directory containing accelerometer data files.")
    parser.add_argument('--limits', '-l', nargs=2, type=int, required=False)
    args = accelerometer_arguments(parser)

    n = 0
    data_list = []
    while os.path.isfile(f"{args.dir}/DATA{n}_accelerometer.csv"):
        data_list.append(np.genfromtxt(f"{args.dir}/DATA{n}_accelerometer.csv", delimiter=",", names=True))
        n += 1
